...
(** This is a convenience API which collects everything needed to create
    {{!Caqti_template.Request} request templates}.
    ...
    {[
      module Ct : sig
        ...
        include module type of (struct include Caqti_template.Create end)
...
(** {2 Request Templates}
    ...
    The following are shortcuts for {!Request.create} and {!Query.parse} In
    ...
    pure and may be recalled repeatedly, cf. {!Request.create}. *)

val static :
  ('a, 'b, 'm) Request_type.t -> string ->
  ('a, 'b, 'm) Request.t
(** Creates a template of static lifetime for prepared requests where the query
    template is provided as a string to be parsed by {!Query.parse}. *)
