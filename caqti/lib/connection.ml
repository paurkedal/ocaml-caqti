...
(** Signature of connection handles.
    ...
    While values of {!Template.Request.t} hold SQL code to be sent to the
    database, connection handles defined here provide the means to execute them
    with actual parameters on an RDBMS.
    So, there is a separation between preparation and execution.
    This is motivated by the common support for prepared queries in database
    client libraries, and by the desire to keep the possibly deeply nested
    data-processing code uncluttered by strings of SQL code.
    For this separation to be reasonably safe, the request declares the types of
    parameters and result, and these type declarations are placed right next to
    the SQL code, so that we can rely on OCaml's powers of refactoring large
    code bases safely.
...
module type Base = sig
  ...
  val call :
    ...
    ('a, 'b, 'm) Template.Request.t -> 'a ->
    ('c, [> Error.call] as 'e) result fiber
  ...
  val set_statement_timeout :
    float option -> (unit, [> Error.call]) result fiber
  ...
  val start : unit -> (unit, [> Error.transact]) result fiber
  (** Starts a transaction if supported by the underlying database, otherwise
      does nothing. *)

  val commit : unit -> (unit, [> Error.transact]) result fiber
  (** Commits the current transaction if supported by the underlying database,
      otherwise does nothing. *)

  val rollback : unit -> (unit, [> Error.transact]) result fiber
  ...
  val deallocate :
    ('a, 'b, 'm) Template.Request.t -> (unit, [> Error.call]) result fiber
...
module type Convenience = sig
  ...
  val exec_with_affected_count :
    ('a, unit, [< `Zero]) Template.Request.t -> 'a ->
    (int, [> Error.call_or_retrieve | `Unsupported]) result fiber
  ...
  val find :
    ...
    ('b, [> Error.call_or_retrieve]) result fiber
  ...
  val find_opt :
    ...
    ('b option, [> Error.call_or_retrieve]) result fiber
  ...
  val fold :
    ...
    ('b -> 'c -> 'c) ->
    'a -> 'c -> ('c, [> Error.call_or_retrieve]) result fiber
  ...
  val fold_s :
    ...
    'a -> 'c -> ('c, [> Error.call_or_retrieve] as 'e) result fiber
  ...
  val iter_s :
    ...
    'a -> (unit, [> Error.call_or_retrieve] as 'e) result fiber
  ...
  val collect_list :
    ...
    ('b list, [> Error.call_or_retrieve]) result fiber
  ...
  val with_transaction :
    ...
    ('a, [> Error.transact] as 'e) result fiber
...
module type Populate = sig
  ...
  val populate :
    table: string ->
    columns: string list ->
    'a Template.Row_type.t -> ('a, 'err) stream ->
    (unit, [> Error.call_or_retrieve | `Congested of 'err]) result fiber
