...
let find_impl msg =
  ...
  with Not_found ->
    Printf.ksprintf failwith
      "Missing call to Caqti.Error.define_msg for (%s _ : Caqti.Error.msg)]"
...
type coding_error = {
  uri: Uri.t;
  typ: Template.Row_type.any;
  msg: msg;
}
let pp_coding_error ppf fmt err =
  Format.fprintf ppf fmt Template.Row_type.pp_any err.typ pp_uri err.uri;
  Format.pp_print_string ppf ": ";
  pp_msg ppf err.msg
...
let encode_missing ~uri ~field_type () =
  let typ = Template.Row_type.Any (Template.Row_type.field field_type) in
...
let encode_rejected ~uri ~typ msg =
  let typ = Template.Row_type.Any typ in
  `Encode_rejected ({uri; typ; msg} : coding_error)
...
let () = Printexc.register_printer @@ function
 | Exn err ->
    Some (show err)
 | Template.Query.Expand_error err ->
    Some (show_of_pp Template.Query.Expand_error.pp err)
 | _ ->
    None
