...
module Config : sig
  ...
  val create :
    ...
    unit -> t
  (** Creates a configuration for the implementation of {!Caqti.Pool.S}.
      The main arguments populate the configuration with the corresponding
      settings, which are explaind in {!cpck}. *)

  val default : t
  (** The configuration object with no setting, which gives the built-in
      defaults.
  ...
  (** [merge_left cL cR] is the configuration [cL] with missing settings
      populated by the corresponding present settings from [cR]. *)
  ...
  (** The maximum number of open connections associated with the pool.
      When this limit is hit, an attempt to use the pool will block until a
      connection becomes available.
      The value must be at least one.
      If the selected driver does not support concurrent connections, the value
      [1] is assumed. *)

  val max_idle_size : int key
  (** The maximum number of idle connections to put into the pool for reuse.
      Must be between [0] and {!max_size}.
      If you set this, you must also ensure {!max_size} is set so that the
      combination is valid.
      Defaults to {!max_size}.
      For drivers which does not support concurrent connections, the value will
      clipped to a maximum of [1]. *)

  val max_idle_age : Mtime.Span.t option key
  (** The maximum age of idle connections before they are scheduled to be
      disconnected and removed from the pool, or [None] for no limit.
      Where possible, a timer will be used to trigger the cleanup.
      For the [caqti.blocking] library, the cleanup will only be done
      opportunistically when the pool is used. *)

  val max_use_count : int option key
  (** The maximum number of times a pooled connection is reused, or [None] for
      no limit.
      The default is currently 100, but may be changed in the future based on
      real-world experience.
      The reason this setting was introduced is that we have seen state being
      retained on the server side. *)
