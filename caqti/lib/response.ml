...
module type S = sig
  ...
  val returned_count :
    ('b, 'm) t -> (int, [> Error.retrieve | `Unsupported]) result fiber
  ...
  val iter_s :
    ...
    ('b, 'm) t -> (unit, [> Error.retrieve] as 'e) result fiber
  ...
  val to_stream : ('b, 'm) t -> ('b, [> Error.retrieve]) stream
