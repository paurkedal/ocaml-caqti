...
let mk_test (name, connect, pool) =
  ...
  let pass_conn (name, speed, f) =
    let f' () =
      match Caqti_miou.Pool.use (fun c -> Ok (f c)) pool with
       | Ok () -> ()
...
let mk_tests sw {uris; connect_config} =
  ...
  let create_target uri =
    let connect () = Caqti_miou_unix.connect ~sw ~stdenv:() ~config:connect_config ~env uri in
    (match Caqti_miou_unix.connect_pool ~sw ~stdenv:() uri
...
let () = Miou_unix.run @@ fun () ->
  Caqti_miou.System.Switch.run @@ fun sw ->
